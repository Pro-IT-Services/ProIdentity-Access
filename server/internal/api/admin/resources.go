...
func (h *ResourceHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if req.Ports != nil {
		...
		h.DB.Exec("UPDATE resources SET ports=? WHERE id=?", normalizedPorts, id)
	}
...
func normalizeResourcePorts(raw *string) (*string, error) {
	if raw == nil {
		return nil, nil
	}
	normalized := ports.Normalize(*raw)
	if normalized == "" {
		return nil, nil
	}
	if err := ports.Validate(normalized); err != nil {
		return nil, err
	}
