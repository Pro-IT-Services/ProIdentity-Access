...
interface MonoChipProps {
  ...
  className?: string
  truncate?: boolean
  /** When true, renders inline (no padding/background) — useful inside dense rows. */
  bare?: boolean
}

/** Monospace value (IP, key, port…) with optional copy-to-clipboard. */
export function MonoChip({ value, display, copy = true, className, truncate, bare }: MonoChipProps) {
  ...
  if (bare) {
    return (
      <span className={cn('inline-flex items-center gap-1 font-mono text-xs', className)}>
        ...
        {copy && (
          <button onClick={onCopy} className="text-muted-foreground/60 hover:text-foreground transition-colors cursor-pointer" aria-label="Copy">
