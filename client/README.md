...
```text
ProIdentity-Access-0.5.20.msi
ProIdentity-Access-0.5.20.pkg
ProIdentity-Access-0.5.20-SHA256SUMS.txt
```

### Windows MSI

Run `ProIdentity-Access-0.5.20.msi`.
...
Run `ProIdentity-Access-0.5.20.pkg`.
...
cd client
powershell -ExecutionPolicy Bypass -File .\build.ps1 -Version 0.5.20
```

Output:

```text
client/build/ProIdentity-Access-0.5.20.msi
```
...
```sh
cd client
./build.sh --version 0.5.20
```

Output:

```text
client/build/darwin/ProIdentity-Access-0.5.20.pkg
```

Install locally:

```sh
sudo installer -pkg client/build/darwin/ProIdentity-Access-0.5.20.pkg -target /
```
