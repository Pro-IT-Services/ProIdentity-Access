...
```text
ProIdentity-Access-0.5.22.msi
ProIdentity-Access-0.5.22.pkg
ProIdentity-Access-0.5.22-SHA256SUMS.txt
```

### Windows MSI

Run `ProIdentity-Access-0.5.22.msi`.
...
Run `ProIdentity-Access-0.5.22.pkg`.
...
cd client
powershell -ExecutionPolicy Bypass -File .\build.ps1 -Version 0.5.22
```

Output:

```text
client/build/ProIdentity-Access-0.5.22.msi
```
...
```sh
cd client
./build.sh --version 0.5.22
```

Output:

```text
client/build/darwin/ProIdentity-Access-0.5.22.pkg
```

Install locally:

```sh
sudo installer -pkg client/build/darwin/ProIdentity-Access-0.5.22.pkg -target /
```
