...
export const useManagedStore = create<ManagedStore>((set, get) => ({
  ...
  loadServers: async () => {
    try {
      ...
      const previous = new Map(get().servers.map(ss => [ss.server.id, ss]))
      const servers: ServerStatus[] = (serverList ?? []).map(s => ({
        server: s,
        connected: activeSet.has(s.id),
        tunnelId: activeSet.has(s.id) ? (previous.get(s.id)?.tunnelId ?? null) : null,
        connecting: false,
        error: null,
      }))
      set({ servers })
  ...
  login: async (username, password, totpCode) => {
    ...
    try {
      ...
      if (result.require_totp) {
        ...
        return {
          requireTOTP: true,
          pushAuthEnabled: result.push_auth_enabled ?? false,
          pushRequestId: result.push_request_id ?? '',
        }
      }
      set(s => ({
        loading: false,
      ...
      return { requireTOTP: false, pushAuthEnabled: false, pushRequestId: '' }
    } catch (e) {
  ...
  loginWithPush: async (username, password, pushRequestId) => {
    set({ loading: true, error: null })
    try {
      const result = await api.managedLoginWithPush(username, password, pushRequestId)
      set(s => ({
        loading: false,
        settings: {
          ...s.settings,
