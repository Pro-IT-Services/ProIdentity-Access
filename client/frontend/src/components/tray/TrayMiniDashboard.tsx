...
import { formatBytes, formatHandshake, formatRate } from '../../lib/format'
...
export function TrayMiniDashboard({
  ...
  const latestSample = history.length ? history[history.length - 1] : undefined
  const rxRate = latestSample?.rxBps ?? 0
  const txRate = latestSample?.txBps ?? 0
          ...
          <div className="flex-1 overflow-hidden px-5 py-4">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  {connected ? `${formatRate(totalRate)} live` : approving ? 'Waiting for 2FA approval' : connecting ? 'Waiting for tunnel approval' : 'Ready when you are'}
                  </p>
                </div>
...
function Metric({
  ...
  return (
    <div className="rounded-lg border border-border bg-card/70 p-2.5">
      ...
      <p className="truncate text-[11px] text-muted-foreground">{rate}</p>
    </div>
  )
}
